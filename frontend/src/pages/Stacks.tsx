...
export function Stacks() {
  ...
  const loadStacks = async () => {
    try {
      setLoading(true);
      const data = await api.listStacks();
      setStacks(data);
    } catch (e: any) {
      message.error('Error loading stacks: ' + e.message);
    } finally {
      setLoading(false);
    }
  };
    ...
    <Layout style={{ minHeight: '100vh' }}>
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleCreate}>
          <Form.Item name="name" label="Name" rules={[{ required: true, message: 'Stack name is required' }]}>
